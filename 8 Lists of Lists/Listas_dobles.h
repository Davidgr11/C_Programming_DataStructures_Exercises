...
struct avioninfo {
  char destino[MAX], nombre[MAX];
  int vuelo;
  N *inicio, *fin;
};typedef  struct avioninfo A;

struct nodoavion{
  A datos;
  struct nodoavion *ant;
  struct nodoavion *sig;
};typedef struct nodoavion ND;
...
int insertar_doble_inicio(ND **inicio, ND **fin, A temporal);
int insertar_doble_final(ND **inicio, ND **fin, A temporal);
int insertar_doble_medio(ND **inicio, ND **fin, A temporal);
...
//////////////////////////////////////////////////////////////////////////////////
int menu_listas_dobles(ND **inicio, ND **fin)
{
    //system("color 4b");
    //ND *inicio=NULL, *fin=NULL;
    char temporal[MAX];
    int opc, exito;
    printf("-----BIENVENIDO A LISTAS DOBLEMENTE LIGADAS-----\n");
    do{//Menu de opciones
        printf("\n1) Insertar nodo\n2) Borrar dato\n3) Recorrer\n4) Buscar\n5) Salir\n");
        opc=validar_num();
        switch(opc)
...
{
    int exito;
    A temporal;//para guardar datos
    temporal=llenarAvion();
...
int insertar_doble_medio(ND **inicio, ND **fin, A temporal)
{
    ...
    if(!nuevo)
        ...
        (aux->sig)->ant=nuevo;//al anterior del siguiente ser� el nuevo
        aux->sig=nuevo;//el que apuntaba a nulo ahora apunta a nuevo
...
int recorrer_doble(ND **inicio, ND **fin)
{
    if(!(*inicio))
        ...
        if(forma==1){
            ...
            {
                printf("Aerolinea: %15s\tDestino: %15s\t Vuelo: %04d\n", aux->datos.nombre, aux->datos.destino, aux->datos.vuelo);
                aux=aux->sig;//apunto aux al nodo siguiente
            }
            ...
            while(aux!=NULL)//si ese nodo no es nulo seguimos recorriendo
            {
                printf("Nombre: %15s\tDestino: %s\tVuelo:%d\n", aux->datos.nombre, aux->datos.destino, aux->datos.vuelo);
...
A llenarAvion(void)  //Funci�n para llenar los datos del nodo
{   
    A avion;
    printf( "Dame la aerolinea\n" );
    scanf( " %30[^\n]s", avion.nombre );
    printf( "Dame el destino\n" );
    scanf( " %30[^\n]s", avion.destino );
    printf( "Dame el numero de vuelo\n" );
    avion.vuelo = validar_num();

		avion.inicio 	= NULL;
		avion.fin 		= NULL;
		
    return avion; //regresamos una estructura de los datos que tiene una persona
}
...
int buscar_doble(ND **inicio, ND **fin, char temporal[MAX])
{
    if(!(*inicio))
        ...
        ND *aux;
        printf("Dame el nombre de la aerolinea\n");
        scanf(" %30[^\n]s", temporal);
        aux=*inicio;
        ...
        if(!aux)
          ...
          printf("Nombre: %s\tDestino: %s\tVuelo:%d\n", aux->datos.nombre, aux->datos.destino, aux->datos.vuelo);
            return 1;
        }
    }
}
